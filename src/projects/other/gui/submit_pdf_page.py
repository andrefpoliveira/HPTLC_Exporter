...
from src.projects.other.reader.hptlc_reader import OtherHptlcReader
...
class SubmitPdfPage(ctk.CTkFrame):
    ...
    def create_window(self):
        ...
        samples_scrollable_frame = ctk.CTkScrollableFrame(self, width = 200, height = 200, border_color="grey", border_width=1)
        samples_scrollable_frame.place(x=570, y=5)

        for id, s in enumerate(self.project["configuration"]["samples"]):
            frame = ctk.CTkFrame(samples_scrollable_frame, width=200, height=25)
            frame.grid(row=id, column=0, pady=1)

            sample_label = ctk.CTkLabel(frame, text=f'{s}', font=("Arial", 12))
            sample_label.place(x=10, y=0)

        groups_title = ctk.CTkLabel(self, text="Groups:", font=("Arial", 15, "bold"))
        groups_title.place(x=570, y=220)

        groups_scrollable_frame = ctk.CTkScrollableFrame(self, width = 200, height = 200, border_color="grey", border_width=1)
        groups_scrollable_frame.place(x=570, y=250)
    ...
    def load_tracks(self):
        ...
        self.reader = OtherHptlcReader(self.pdf_file_entry.get(), ProjectTypeEnum.get_regex(self.project["type"]), self.project["folder"] + "/" + self.project["title"] + ".xlsx")
        ...
        checked = [False for _ in self.tables]
